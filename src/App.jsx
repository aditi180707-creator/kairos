      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={S.card}>
          ...
          <input value={name} onChange={e => setName(e.target.value)}
            ...
            placeholder="" style={{ ...S.input, marginBottom: 24 }} autoFocus />
          <button onClick={() => ok && onEnter(name.trim())} disabled={!ok}
...
function CaptureScreen({ name, onDone }) {
  ...
  const runCountdown = useCallback(() => new Promise(resolve => {
    let t = timerDuration; setCountdown(t);
    const iv = setInterval(() => {
      t--; if (t === 0) { clearInterval(iv); setCountdown(null); resolve(); } else setCountdown(t);
    }, 1000);
  }), [timerDuration]);
  ...
  const start = async () => {
    ...
    // Pass photos + settings to parent
    onDone(captured, selectedFilter, selectedFrame, filterCss, frame, frameBoxBg);
  };
            ...
            {photos.length > 0 && (
              <div style={S.card}>
                ...
                <div style={{
                  ...
                  {photos.map((p,i) => <img key={i} src={p} alt="" style={{ width:"100%", borderRadius:5, display:"block", filter:filterCss }} />)}
...
function ResultScreen({ name, photos, filterCss, frame, frameBoxBg, onRetake }) {
  ...
  useEffect(() => {
    ...
    } else { ctx.fillStyle = frame.bg || "#fff"; }
    ctx.fillRect(0, 0, sc.width, sc.height);
    if (frame.border && frame.border !== "none") {
      ctx.strokeStyle = frame.border; ctx.lineWidth = 3;
    ...
    Promise.all(photos.map((src, i) => new Promise(res => {
      ...
      img.onload = () => {
        ...
        ctx.filter = "none";
        if (frame.border && frame.border !== "none") {
          ctx.strokeStyle = frame.border; ctx.lineWidth = 1; ctx.strokeRect(pad, y, iw, ih);
        }
        res();
      };
      img.src = src;
      ...
      ctx.fillText(new Date().toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"}), sc.width/2, sc.height - footer + 42);
      ctx.globalAlpha = 1;
  ...
  const download = () => {
    ...
    a.download = `kairos-${Date.now()}.jpg`; a.click();
  };
        ...
        <div style={{ display: "flex", gap: 24, alignItems: "flex-start", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "0 0 340px", minWidth: 280, ...S.card }}>
            ...
            <div style={{ display:"flex", gap:10, marginBottom:10 }}>
              <button onClick={download} style={{ ...S.btnPrimary, flex:1, padding:"13px 0" }}>
                {stripDataUrl ? "Download" : "Building…"}
              </button>
              <button onClick={onRetake} style={{ ...S.btnSecondary, flex:1, padding:"12px 0" }}>Retake</button>
            </div>
          ...
          <div style={{ flex:"1 1 300px", minWidth:260, display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={S.card}>
              ...
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:16 }}>
                <QRImg value={shareLink} size={160} />
...
export default function App() {
  ...
  const [result, setResult]   = useState(null); // {photos, filterCss, frame, frameBoxBg}
  ...
  return (
    <CaptureScreen
      name={name}
      onDone={(photos, filter, frameId, filterCss, frame, frameBoxBg) => {
        setResult({ photos, filter, frameId, filterCss, frame, frameBoxBg });
      }}
    />
  );
}